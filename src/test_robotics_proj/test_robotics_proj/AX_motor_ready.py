...
class MotorReady(Node):
    def __init__(self):
        ...
        self.gripper_sub = self.create_subscription(Float32, '/robotics/gripper/command', self.gripper_callback, 10)
        ...
        for motor_id in AX_ID_LIST:
            ...
            # 초기 속도 제한 설정
            limit_speed = speed_limits.get(motor_id, 400)
    ...
    def gripper_callback(self, msg):
        data = msg.data
        with self.lock:
            # 그리퍼 모터 (XC 모터) 처리: write4ByteTxRx 사용
            dxl_comm_result, dxl_error = self.xc_packet.write4ByteTxRx(self.xc_port, XC_ID, XC_GOAL_POSITION_ADDR, int(data))
            if dxl_comm_result != COMM_SUCCESS:
                self.get_logger().warn("XC Motor: Gripper Goal Position 쓰기 실패({})".format(self.xc_packet.getTxRxResult(dxl_comm_result)))
